...
LaserBotTerranFrag = Class(TLaserBotProjectile)  {
    OnImpact = function(self, TargetType, TargetEntity) 
        
        local FxFragEffect = EffectTemplate.TFragmentationSensorShellFrag 
        local ChildProjectileBP = '/mods/BlackOpsFAF-ACUs-YYPatch/projectiles/NapalmProjectile01/Napalm01_proj.bp'  
        ...
        local angleInitial = RandomFloat( 0, 360 )
        
        # Randomization of the spread
        local angleVariation = angle * 1 # Adjusts angle variance spread
        local spreadMul = 1 # Adjusts the width of the dispersal        
        
        local xVec = 0 
        local yVec = math.abs(vy) * 1.5
        local zVec = 0
